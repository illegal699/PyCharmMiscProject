...
class MetaAgentConfig:
    ...
    locked_indicators: dict = None  # {indicator: (is_locked, enabled)}

    def __post_init__(self):
        if self.locked_params is None:
            self.locked_params = {}
        if self.locked_indicators is None:
            self.locked_indicators = {}
...
class MetaAgent:
    ...
    def _run_optuna(self):
        ...
        if existing > 0:
            ...
            try:
                ...
                self._update(
                    f"🏆 Najlepszy poprzedni wynik: score={self.progress.best_score:.4f} (trial #{best.number + 1})")
            except Exception:
                pass
        else:
        ...
        for trial_num in range(self.mc.n_trials):
            ...
            self.progress.history.append({
                ...
                "balance": metrics.get("final_balance", self.tc.initial_balance),
            })
        ...
        self._update(
            f"🏁 Optuna zakończona. Best score: {self.progress.best_score:.4f} (trial {self.progress.best_trial})")
    ...
    def _run_trial(self, params: dict, trial_num: int) -> tuple[float, dict]:
        ...
        def on_episode(ep_num, total_eps, stats=None):
            pct = ((trial_num * total_eps + ep_num) /
                   max(self.mc.n_trials * total_eps, 1)) * 100
            self.progress.pct_complete = min(pct, 100.0)
            # Aktualizuj metryki bieżącego epizodu w logu
            if stats:
                ret = stats.get("total_return_pct", 0)
                bal = stats.get("final_balance", self.tc.initial_balance)
                trd = stats.get("total_trades", 0)
                wr = stats.get("win_rate", 0)
                sh = stats.get("sharpe", 0)
                dd = stats.get("max_drawdown_pct", 0)
                msg = (f"Trial {trial_num + 1}/{self.mc.n_trials} | "
                       f"Ep {ep_num}/{total_eps} | "
                       f"Ret:{ret:+.2f}% | Bal:${bal:,.2f} | "
                       f"Trades:{trd} | WR:{wr:.0f}% | "
                       f"Sharpe:{sh:.2f} | DD:{dd:.1f}%")
    ...
    def _suggest_params(self, trial) -> dict:
        ...
        # Helper dla wskaźników — zablokowane lub sugerowane przez Optuna
        li = self.mc.locked_indicators

        def _ind(name, key):
            e = li.get(name)
            return e[1] if (e and e[0]) else trial.suggest_categorical(key, [True, False])

        # Cechy — wskaźniki cenowe losowane, reszta zawsze włączona
        features = {
            "use_ema_9": _ind("ema", "use_ema_9"),
            "use_ema_21": _ind("ema", "use_ema_21"),
            "use_ema_50": _ind("ema", "use_ema_50"),
            "use_ema_cross": _ind("ema", "use_ema_cross"),
            "use_rsi_1m": _ind("rsi", "use_rsi_1m"),
            "use_rsi_5m": _ind("rsi", "use_rsi_5m"),
            "use_rsi_15m": _ind("rsi", "use_rsi_15m"),
            "use_macd_hist": _ind("macd", "use_macd_hist"),
            "use_macd_cross": _ind("macd", "use_macd_cross"),
